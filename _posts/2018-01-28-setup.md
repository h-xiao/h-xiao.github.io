...
def get_fundamental_data(sp_ticker_list, is_list, cf_list, bs_list, index_dict, tickers_dict):
    ...
    for i in range(len(sp_ticker_list)):
        ...
        try:
            ...
            all_statement_data_qt = yf.get_financial_stmts(index_dict['type'], ['income', 'cash', 'balance'])
            ...
            df = pd.merge(df, df_template, how="outer")  # merge to insert the additional columns that are missing
    ...
    errors_df.to_csv(os.path.join(paths['csv'], 'get_fundamental_data_errors.csv'),index=False)
